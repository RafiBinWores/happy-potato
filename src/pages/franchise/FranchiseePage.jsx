  ...
  return (
    <>
      ...
      <div className="c-space py-8 md:py-20 z-20 relative">
        <div className="grid grid-cols-1 lg:grid-cols-5 lg:gap-10 md:pb-7 lg:pb-40 mt-5 md:mt-10 gap-y-6">

          {/* LEFT PANEL */}
          <div className="col-span-2 relative overflow-hidden">
           <div className="bg-primary  rounded-2xl p-5">
             <h4 className="text-secondary text-[20px] md:text-[26px] font-headingNew uppercase leading-tight">
              Interested in Franchising <br /> with Happy Potato?
            </h4>

            <h5 className="mt-6 text-white font-semibold">
              Happy Potato benefit
            </h5>

            <ul className="mt-4 space-y-3 text-white text-sm md:text-base">
              <li className="flex gap-3 items-start">
                <img src="/assets/images/franchise/check.png" alt="check icon" className="w-6" />
                <p>Top-quality product with affordable price</p>
              </li>
              <li className="flex gap-3 items-start">
                <span className="mt-1 size-5 rounded-full bg-secondary text-primary grid place-items-center font-bold">
                  ✓
                </span>
          ...
          <div className="col-span-3 bg-white rounded-4xl p-[22px] md:p-[50px] shadow-[0_4px_17px_5px_rgba(0,0,0,0.25)]">
            <p className="text-[12px] md:text-[14px] text-black/80 leading-relaxed mb-5">
              Greetings from Happy Potato Malaysia! We would like to extend our warmest
              appreciation for your interest for us. In order for us to proceed further with
              your application, kindly provide us some information by filling in the below:
            </p>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 md:space-y-5">
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
                ...
                <div className="w-full">
                  <input
                    type="text"
                    className={`${inputBase} ${errors.name ? "ring-2 ring-red-500" : ""}`}
              ...
              {/* NRIC */}
              <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
                <label className={labelBase}>NRIC</label>
                <div className="w-full">
                  <input
                    type="text"
                    className={`${inputBase} ${errors.nric ? "ring-2 ring-red-500" : ""}`}
                    {...register("nric", { required: "NRIC is required" })}
                  />
                  {errors.nric && <p className={errText}>{errors.nric.message}</p>}
                </div>
              </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
                ...
                <div className="w-full">
                  <input
                    type="text"
                    className={`${inputBase} ${errors.address ? "ring-2 ring-red-500" : ""}`}
              ...
              <div className="pt-1">
                ...
                <div className="flex items-center gap-8">
                  <label className="flex items-center gap-2 text-sm text-customBlue">
                    <input
                      ...
                      {...register("experience", { required: "Select yes or no" })}
                    />
                    Yes
                  </label>
                  <label className="flex items-center gap-2 text-sm text-customBlue">
                ...
                {errors.experience && <p className={errText}>{errors.experience.message}</p>}
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  {...register("experienceDetails", {
                    validate: (val) =>
                      experienceValue !== "yes" || val.trim().length > 0 || "Please describe your experience",
                  })}
                />
                {errors.experienceDetails && (
                  <p className={errText}>{errors.experienceDetails.message}</p>
                )}
              </div>
              ...
              <div>
                <p className="text-customBlue font-semibold text-[13px] md:text-[15px] mb-2">
                  Which City or Area are you interested to set up a Happy Potato Outlet?*
                </p>
                <input
                  type="text"
                  className={`${inputBase} ${errors.city ? "ring-2 ring-red-500" : ""}`}
              ...
              <div className="pt-1">
                ...
                <div className="flex items-center gap-8">
                  <label className="flex items-center gap-2 text-sm text-customBlue">
                    <input
                      ...
                      value="yes"
                      className="accent-primary"
                      {...register("selfOperate", { required: "Select yes or no" })}
                    />
                    Yes
                  </label>
                  <label className="flex items-center gap-2 text-sm text-customBlue">
                ...
                {errors.selfOperate && <p className={errText}>{errors.selfOperate.message}</p>}
              </div>

              {/* Investment */}
              <div>
                <p className="text-customBlue font-semibold text-[13px] md:text-[15px] mb-2">
                  What is the total amount you expect to invest in starting this business?*
                </p>
                <input
                  type="text"
                  className={`${inputBase} ${errors.investment ? "ring-2 ring-red-500" : ""}`}
                  {...register("investment", { required: "Investment amount is required" })}
                />
                {errors.investment && <p className={errText}>{errors.investment.message}</p>}
              </div>
